...
const sample = getSample('apples-new-gravity')!;
const site = 'https://snwhip.com';
const url = `${site}/writing/apples-new-gravity`;
...
export default function ApplesNewGravityPage() {
  return <>
      <Helmet>
        <title>{title}</title>
      ...
      <main>
        ...
        {/* ── Works Cited ── */}
        {sample.worksCited && sample.worksCited.length > 0 && <motion.div initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
        ...
        delay: 0.26,
        ease: 'easeOut' as const
      }} className="max-w-3xl mx-auto px-6 md:px-10 pt-12 pb-16 md:pb-24">
          
            <hr className="border-t border-border mb-8" />
            <p className="text-xs font-medium tracking-[0.2em] uppercase text-muted-foreground mb-6" style={{
          fontFamily: 'var(--font-sans)'
        }}>
            
              Works Cited
            </p>
            {sample.worksCited.map(entry => <p key={entry.id} className="text-sm text-foreground/60 leading-relaxed mb-4 last:mb-0" style={{
          fontFamily: 'var(--font-sans)',
          fontWeight: 300
        }}>
